...
flowchart LR
    ...
    S3["cryptomcp-bot<br/>Telegram: кнопки сценариев"]
    TG["Telegram"]
    LLM["модель"]
...
| | сервер | сборщик | бот |
|---|---|---|---|
| Промолчал час | никто не пострадал, следующий вопрос обслужен | потерян час открытого интереса **навсегда** | кнопки не отвечают, данные целы |
| Что делает | отвечает, ничего не копит | копит, ни на что не отвечает | отвечает кнопками, копит только свои расходы |
| Доступ к архиву | только чтение | единственный писатель | только чтение |
| Своя база | `watchlist_manual.sqlite` — ручные записи списка | — | `bot.sqlite` — расходы, чаты, сессии |
| Смотрит наружу | да, порт за Caddy | нет, портов не публикует | нет, сам ходит в Telegram |
| Знает токен | да | токен Telegram | токен Telegram и ключ Claude |

**Бот — третий процесс по той же причине.** Бесконечный цикл `getUpdates`,
зависший на сети, не должен утянуть ни ответы сервера, ни часовой прогон, а
признак живости у него свой: «когда последний раз успешно получены
обновления». Сценарии он считает через Claude и платит за них деньгами, и
дневной лимит с учётом расходов живёт в его собственной базе (PLAN §4.44).
...
последовательность REST-снимков, diff без уровней за дальним краем окна и
