...
flowchart LR
    ...
    S1["cryptomcp<br/>MCP-сервер, 9 инструментов"]
    TG["Telegram"]
    LLM["модель"]
...
| Доступ к базе | только чтение | единственный писатель |
...
flowchart TD
    ...
    B -->|"оборот от 10M"| ARCH["архивный слой:<br/>1d/4h/1h"]
...
`squeeze_index` — свёртка пяти групп признаков, каждая в диапазоне 0…1.
...
| `value_area` | 0.10 | положение цены внутри Value Area объёмного профиля |
| `divergence` | 0.10 | дивергенции RSI на окне 40 свечей |
...
Список ведётся на 4h и 1d. На 1h индекс считается и пишется в журнал, но
эпизодов не открывает: третий ТФ поднял бы потолок списка с 80 эпизодов до 120,
а список и без того признан длинным.
...
| L2 | `get_squeeze_metrics` | пять групп признаков с базой сравнения |
...
`get_watchlist`, `get_scan_history`. Каждый работает и по фьючерсам, и по споту:
