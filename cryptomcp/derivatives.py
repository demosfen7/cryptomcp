...
def classify_price_oi(
    ...
    Помимо четырёх основных квадрантов различаются два вырожденных случая, и
    они не менее содержательны. Заметный рост OI при стоящей цене — это то, что
    ТЗ §4.2 называет «набор позиции без движения цены», главный признак
    накопления. Обратное — движение цены без изменения OI — означает переход
    ...
    if oi_moved and not price_moved:
        return "набор позиций без движения цены"
