...
#:
#: ТЗ называет 2.0x, но при нём его же приёмочный тест 3 не воспроизводится:
#: в окне OAX на 20.10.2024 проходят ровно два бара (09.10 — 41.74x при фитиле
#: 69% и 13.10 — 47.50x при 51%), а ждётся `confirmed`, то есть три события.
#: Ближайшие кандидаты — 02.10 (1.58x, фитиль 65%) и 07.10 (0.95x, 69%), и
#: третье событие появляется при пороге около 1.6x. Значение выбрано
#: заказчиком после этого замера; §10 ТЗ называет все пороги стартовыми.
VOLUME_MULTIPLE = 1.6
...
def _turnover_per_day(series: Series, window: int) -> float:
    ...
    оборота на дату в прошлом не существует.

    Среднее по окну, а не медиана, и это не вкус. Раздел 2 той же выдачи уже
    печатает «оборот окна N USDT за 30 свечей · M на свечу», и это среднее.
    Медиана давала второе число под тем же словом: на AINUSDT 15.09.2026
    раздел 2 показывал 5.19M на свечу, а раздел 7 рядом — «оборот 0.66M ниже
    2M», потому что монета взорвалась только в последние двое суток из
    тридцати. Две величины под одним именем в одном сообщении читаются как
    два разных факта — ровно та ошибка, которая в проекте чинилась уже трижды.
    """
