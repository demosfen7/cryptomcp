...
#: свечей до первого значения. Живёт здесь, а не в `reader`, потому что это
#: свойство индикаторов, а не источника данных; `reader` его отсюда и берёт.
WARMUP = 250
...
def canonical_span_days(interval: str) -> float:
    ...
    На 4h это 611 свечей, то есть 102 суток, и число это одинаково для всех
    монет: оно свойство таймфрейма, а не символа.
    """
    from .series import interval_ms

    return (required_candles(interval) + WARMUP) * interval_ms(interval) / 86_400_000


def duration_span_bar(interval: str, configured: float) -> float:
    """Требование к охвату для группы «длительность» — с потолком в окно.

    **`min`, а не голая константа, и упрощать обратно нельзя.** Правило
    «охват не менее 180 суток» задумано как «у ЭТОЙ монеты достаточно
    истории». На 1d оно так и работает: окно 610 суток шире порога, и порог
    отбраковывает реальные символы (18 из 30 в замере 04.09.2026). На 4h
    окно — ровно 102 суток у всех разом, порог стоит выше этой константы, и
    условие превращается в безусловное «нет»: 0 из 30, причём независимо от
    истории символа. Оно начинает проверять параметр системы вместо свойства
    монеты.

    Потолок возвращает условию исходный смысл. На 4h порог схлопывается до
    полного окна, то есть до «ряд не обрезан», и продолжает ловить ровно то,
    ради чего заводился, — свежие листинги (в том же замере два символа с
    охватом 3 суток). Требование к числу завершённых серий при этом не
    трогается, поэтому зернистость перцентиля не деградирует нигде.

    Планка — окно МИНУС одна свеча. Ряд, собранный из ответа биржи, теряет
    последнюю свечу как незакрытую и приходит на одну короче архивного: 610
    вместо 611 на 4h. Планка ровно в окно отвергала бы такой ряд по разнице в
    четыре часа — тот же тавтологический отказ, только по офф-бай-ван.

    Тот же класс ошибки, что в §4.31: правило, записанное в сутках, молча
    оказалось константой таймфрейма. См. §4.34.
    """
    from .series import interval_ms

    step_days = interval_ms(interval) / 86_400_000
    return min(configured, canonical_span_days(interval) - step_days)
...
def analyse_timeframe(
    ...
    *,
    ...
    narrow_metric = with_percentile(
        ...
        min_span_days=duration_span_bar(
            series.interval, config.duration_min_span_days
        ),
    )
