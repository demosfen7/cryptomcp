...
def render_order_book_watches(watches: Sequence[Mapping[str, Any]], *, now_ms: int) -> str:
    """Короткий список временных сессий с явной отметкой молчащего тикера."""
    if not watches:
        return "Сессий наблюдения за стаканом нет."
    lines = ["Сессии наблюдения за стаканом:"]
    for row in watches:
...
def _watch_status(row: Mapping[str, Any], now_ms: int) -> str:
    ...
    if status != "active":
        return status
    if now_ms >= int(row["ends_at"]):
        return "active, срок окончен — тикер молчит до уборки сборщиком"
    last = row.get("last_snapshot_at")
    stale_after = max(2 * int(row["interval_sec"]) * 1000, 10_000)
    if last is None or now_ms - int(last) > stale_after:
        return "active, но нет свежих снимков — тикер сервера мог остановиться"
    return "active"


def _render_watch_snapshot(snapshot: Mapping[str, Any]) -> list[str]:
    lines = [
        f"  {utc(int(snapshot['ts']))} UTC · mid {snapshot['mid_price']:g} · "
        f"bid {snapshot['best_bid']:g} · ask {snapshot['best_ask']:g} · "
        f"spread {snapshot['spread_pct']:.3f}%",
        "    bids:",
    ]
    lines += [_render_watch_level(level) for level in snapshot["bids"]]
    lines.append("    asks:")
    lines += [_render_watch_level(level) for level in snapshot["asks"]]
    return lines


def _render_watch_level(level: Mapping[str, Any]) -> str:
    return (
        f"      {level['price']:g} · qty {level['qty']:g} · "
        f"notional {usdt(float(level['notional_usdt']))} · "
        f"cum {usdt(float(level['cum_notional_usdt']))}"
    )
...
def _quadrant(value: Any) -> str:
    """Квадрант потока одним словом — в строку таблицы длинный не влезает."""
    return {
        "поглощение": "погл",
        "распределение": "распр",
        "обычный спрос": "спрос",
...
def _trend(ema_state: Any) -> str:
    """Где цена относительно EMA50/EMA200 — словом и коротко.

    Пометка, а не фильтр (PLAN §4.41): измеренная разница между «ниже обеих» и
    остальными есть, но выборка — две недели одного режима рынка.
    """
    return {"below": "ниже", "above": "выше", "mixed": "смеш"}.get(str(ema_state), "—")
...
def render_accumulation(
    ...
    *,
    ...
    Колонки те же по смыслу, что в списке наблюдения, но первым идёт кластер:
    он основание отбора, а индекс здесь только расставляет очередь. Замер
    17.09.2026: верхушка индекса на исходах не отличается от базы, а кластер
    даёт 51% исходов от +10% за 72ч против 39% без него.
    """
    if not entries:
        return (
            "список накопления пуст: монет с кластером набора на младшем ряду "
            "в последнем прогоне не нашлось"
        )

    closed = any(row.get("exited_at") for row in entries)
    lines = [
        f"записей: {len(entries)}" + (f" · формула {version}" if version else ""),
        f"{'символ':<14}{'ТФ':>4}{'рынок':>7}{'ранг':>10}{'инд':>7}"
        f"{'клст':>6}{'бары':>6}{'тренд':>7}{'поток²':>8}"
        f"{'вход':>13}{'сейчас':>9}{'дней':>6}"
    ...
    for row in entries:
        ...
        line = (
            ...
            f"{f'{index_now:.2f}' if index_now is not None else '—':>7}"
            f"{_cell(row.get('last_clusters') or row.get('entered_clusters')):>6}"
            f"{_cell(row.get('entered_bars')):>6}"
            f"{_trend(scan.get('ema_state')):>7}"
            f"{_quadrant(scan.get('twin_delta_quadrant')):>8}"
            f"{_price(price_in):>13}{move:>9}"
            f"{(until - row['entered_at']) / 86_400_000:>6.1f}"
        )
    ...
    lines += [
        "",
        "отбор: кластер набора на младшем ряду есть, ранг по индексу среди "
        "таких монет — вход в топ-15, выход из топ-25. Тишина здесь не "
        "основание отбора, а способ расставить очередь",
        "клст — кластеров набора в последней записи скана (серия свечей, за "
        "которую цена никуда не ушла); бары — одиночные бары набора на входе",
        "ранг — «при входе → сейчас» среди монет с кластером, а не по всему "
        "универсуму: это другой список, и ранги двух списков не сравниваются",
        "список ведётся параллельно списку наблюдения и ничего в нём не "
        "меняет. Смысл в том, чтобы через два-три месяца сравнить по outcomes, "
        "какой отбор оказался полезнее: тишина или признак набора",
        "выход из списка — исчез кластер или ранг ниже 25; пробой здесь не "
        "отслеживается, для этого есть список наблюдения",
    ]
    return "\n".join(lines)
...
def render_watchlist(
    ...
    *,
    ...
    lines = [
        ...
        f"{'инд':>7}{'Δинд':>7}"
        f"{'накопл':>8}{'тренд':>7}{'узк':>5}{'сут':>6}{'узк²':>6}{'поток²':>8}"
        f"{'вход':>13}{'сейчас':>9}{'дней':>6}  кем"
        + ("  ·  чем кончилось" if closed else ""),
    ]
    ...
    lines += [
        ...
        "поток² — квадрант потока тейкеров на соседнем рынке за 30 свечей: "
        "погл — продают в рынок, а цена растёт; распр — покупают в рынок, а "
        "цена падает; спрос и выход — обычные. Считается там же, где узк², и "
        "нужен потому, что объёмные признаки основного ряда берутся с рынка, "
        "где торговли меньше: у 152 архивных монет из 183 спот даёт меньше "
        "30% оборота фьючерсов (замер 17.09.2026). Справка, в ранг не входит",
        ...
        "тренд — где цена относительно EMA50 и EMA200 последнего скана: "
        "ниже обеих, выше обеих или смешанно. Это ПОМЕТКА, а не фильтр: у "
        "верхушки индекса «ниже обеих» дало 40% исходов от +10% за 72ч против "
        "51% у «смешанно» на дневке и 18% против 32–39% на 4h (замер 17.09.2026, "
        "две недели, один режим рынка). А вот структура нижних минимумов НЕ "
        "хуже — вытряска перед разворотом выглядит именно так, и фильтровать "
        "по ней значило бы выбрасывать такие случаи",
    ...
    *,
    ...
    if not rows:
        ...
        reason = (
            ...
            "min_narrow_bars, min_vol_ratio, max_abs_change_window, границы "
            "оборота или max_abs_change_24h."
        )
        return "\n".join(head + ["", reason])

    head.append(
        f"{'символ':<14}{'индекс':>7}{'BBW':>6}{'диап':>8}{'узк':>5}{'объём':>8}"
        f"{'МА':>7}{'расш':>7}{'дельта%':>9}{'шок':>6}{'погл':>6}{'клст':>6}"
        f"{'tkМакс':>8}{'лид':>6}{'фанд%':>9}  поток по OI за сутки"
    )
    lines = list(head)
    for row in rows:
        index = row.get("squeeze_index")
        ...
        lines.append(
            ...
            f"{f'{volume:.2f}x' if volume is not None else 'n/a':>8}"
            f"{_cell(row.get('ma_ratio'), '.2f'):>7}"
            f"{_cell(row.get('vol_ratio_12_30'), '.2f'):>7}"
            f"{_cell(row.get('delta_share_30'), '+.1f'):>9}"
            f"{_shock(row):>6}"
            f"{_cell(row.get('absorption_bars')):>6}"
            f"{_cell(row.get('absorption_clusters')):>6}"
            f"{_cell(row.get('taker_max'), '.2f'):>8}"
    ...
    lines += [
        ...
        "(серия свечей, за которую цена никуда не ушла) · tkМакс — максимум "
        "доли тейкер-покупок · лид — на сколько свечей объём опередил цену",
        "расш — оборот последних 12 свечей к предыдущим 30 (не MA20/MA100: у "
        "той неполный знаменатель на короткой истории) · дельта% — накопленный "
        "поток тейкеров за 30 свечей в долях оборота; отрицательная дельта при "
        "растущей цене и есть поглощение",
